...
- **Sticky-bomb row/column removal never changes the grid dimensions.** Removed rows/cols are flagged per cell (`Grid.<IsSkipRow>/<IsSkipCol>`), tracked as index lists on `GridAccessor` (`get_SkipRow()`/`get_SkipCol()`), and restored over time via `expansionRow()`/`expansionCol()` as each bomb wears off. `GRID_ACTUAL_SIZE_X/Y` keeps reporting the full size, so a reader that ignores the skip flags renders rows that no longer exist — and a structural-change check based on dimensions misses both the removal *and* the gradual restore. The cell reader compacts skipped rows/cols out of the snapshot (which also makes the structural signature catch every change), and move dispatch steps over skipped indices when translating a directional move to an absolute cell write.
- **The red "error nodes" are not a `_GridType`.** Cells carrying the red warning-triangle hazard read as plain `None` in `_GridType`; the live marker is the per-cell field `app.PuzzleSnake.Grid._ObstacleReasons`, a bitmask of `app.PuzzleSnake.ObstacleReason` (`ObstacleGrid=1` — the red nodes, verified in-game — plus `DeadFilament=2`, `Ch16092=4`, `Ch14100=8`, `AllPassed=16`). Nonzero means the cell is *currently impassable*; the engine can set/clear bits mid-fight, so treat it as dynamic state, not authored layout. Same layered-decoration pattern as `_IsGoldenPath`. If your cell reader keys only off `_GridType`, these nodes are invisible — and a `type != None` filter in a debug dump hides them too, which is exactly how this stayed unnoticed for a while. (The adjacent `Grid._DeadFilamentType` field — an `app.DeadFilamentType` hash, `None`/`Random`/`Fixed` — reads as `None` on standard grids and appears to be authored only for dead-filament boss content.)
